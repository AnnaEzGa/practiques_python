...
# Serveix per calcular àreas de objectes i tal...
...
def mc_pi(total_random_points):
    ...
    xc = [] #Els puts que estan dins del cercle: xc i yc
    yc = []

    xs = [] #Els punts que estan fora del cercle: xs i ys
    ys = []
    ...
    while (i<total_random_points):
        ...
        if x**2 + y**2 <= 1.0: #Si es compleix aquesta condició, significa que pertany al cercle
            xc.append(x)
            yc.append(y)
        else: # Tot el que està a sota del "else", és el que es compleix si NO estàn dins dels cercle els números.
            xs.append(x)
            ys.append(y)
...
# ### Plotting
# Function: plot the square, the circle and the generated points (in different colors inside and outside). Estem dibuixant aquí els números que queden dins del cercle d'un color, els que no, d'un altres...
...
total = int(input("\nEnter number of points for Monte Carlo estimate of Pi?\n>")) #PAra poner la cantidad de puntos que queremos poner para estudiar cuantos de esa cantidad, entra dentro del círculo ese.
